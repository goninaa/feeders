...
class AllBats:
    ...
    def save_csv (self):
        root = f'/Users/gonina/Dropbox/feeders_exp/analysis/all_bats_files'
        if not os.path.exists(root):
            os.makedirs(root)
        self.df_all_bats.to_csv(f"{root}/all_bats_{time.strftime('%d-%m-%Y')}.csv")
...
if __name__ == "__main__":
    ...
    # all_bats = AllBats(df_list=bat_list)
    # all_bats.run()
    pass