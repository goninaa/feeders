...
class Feeder:
    ...
    def __init__(self, fname, bat_name):
        self.df = pd.DataFrame(columns=['signal','pump_1','pump_2','bat1_id','bat1_loc',
                                         'bat2_id','bat2_loc','bat3_id','bat3_loc','bat4_id','bat4_loc',
                                         'bats_condition'])
        self.disconnect = []
        # self.bat = False
        # self.cond = None
        ...
        self.activity = False
        self.bats_ids = data.bats_ids #check
        self.bat1_id = None #check
        self.bat2_id = None
        self.bat3_id = None
        self.bat4_id = None
        self.loc_bat1 = None
    ...
    def signal_on_reward (self, rewarding_feeder = 'R', intervals = 20, running_time = 3200, R_p=(1,0), L_p=(1,0)):
        ...
        while time.time() < tr_end:
            ...
            while self.activity == False:
                ...
                bat_dict = {'time': pd.Timestamp.now().strftime('%d-%m-%Y-%H:%M:%S'), 
                            'signal': 'play',
                            'bat1_id': self.bat1_id, 'bat2_id': self.bat2_id, #need to check
                            'bat1_loc': self.loc_bat1, 'bat2_loc': self.loc_bat2,
                            'bat1_condition': self.cond, 'bat2_condition': self.cond}

                
                self.df = self.df.append(bat_dict, ignore_index=True)
    ...
    def read_rfid (self): #need a fix
            # print ("read rfid")
            # print (self.bat_loc)
             ### has a problem- when calling this class all dict becomes empty
            data = DATA(self.fname)
            data.run()
            self.activity = data.activity
            self.bats_ids = self.bats_ids.update(data.bats_ids) #check
            # self.bats_ids = data.bats_ids #check
            self.bat1_id = data.bats_ids.get(1) #check
            self.bat2_id = data.bats_ids.get(2)
            self.bat3_id = data.bats_ids.get(3)
            self.bat4_id = data.bats_ids.get(4)
            self.loc_bat1 = data.bats_loc.get(self.bat1_id)#check
            self.loc_bat2 = data.bats_loc.get(self.bat2_id)
            self.loc_bat3 = data.bats_loc.get(self.bat3_id)
            self.loc_bat4 = data.bats_loc.get(self.bat4_id)
            ...
            for bat in self.bats_ids:
                self.df.loc[pd.Timestamp.now().strftime('%d-%m-%Y-%H:%M:%S'),self.df[f'bat{bat}_id']] = self.bats_ids.get(bat)
                

            bat_dict = {'time': pd.Timestamp.now().strftime('%d-%m-%Y-%H:%M:%S'), 
                        'bat1_id': self.bat1_id, 'bat2_id': self.bat2_id,
                        'bat1_loc': self.loc_bat1, 'bat2_loc': self.loc_bat2,
                        'bat1_condition': self.cond, 'bat2_condition': self.cond}
            self.df = self.df.append(bat_dict, ignore_index=True)
            self.df.to_csv(f"{pd.Timestamp.now().strftime('%Y-%m-%d')}_{self.bat_name}.csv")
    ...
    def pump_it (self, pump_id, win_lose_p = (1,0)):
        ...
        try:
            ...
            #     print ("no luck!")
                ...
                bat_dict = {'time': pd.Timestamp.now().strftime('%d-%m-%Y-%H:%M:%S'), 
                            f'pump_{pump_id}': 'no {}'.format(pump_id),
                            'bat1_id': self.bat1_id, 'bat2_id': self.bat2_id,
                            'bat1_loc': self.loc_bat1, 'bat2_loc': self.loc_bat2,
                            'bat1_condition': self.cond, 'bat2_condition': self.cond}
                print ("no luck!")
