...
class DATA:
    ...
    def check_activity (self):
        ...
        if row_time[0] >= second_ago and ((103 in self.df.last('1s')['ANTENNA'].unique())or(104 in self.df.last('1s')['ANTENNA'].unique()) ): 
        # or self.df.last('1s')['ANTENNA']==102):
            self.activity = True
            # print ('bat')
        # elif row_time[0] >= ten_sec_ago and row_time[0] < second_ago: ##changed
        elif (row_time[0] >= ten_sec_ago 
            and ((103 in self.df.last('10s')['ANTENNA'].unique())
            or (104 in self.df.last('10s')['ANTENNA'].unique()))
