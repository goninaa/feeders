...
class DATA:
    ...
    def __init__ (self,fname):
        ...
        self.tags = [] # list of active tags
        self.bats_ids = {} #dict of bats with tags
        self.tags_ids = {}
    ...
    def check_activity (self):
        ...
        if row_time[0] >= second_ago:
            self.activity = True
        elif row_time[0] >= ten_sec_ago & row_time[0] < second_ago: ##changed
            self.activity = "bat in last 10" ##changed
        else:
            self.activity = False  #false only if was no bat for 10 sec

    def find_bat(self):
        """ find next to which antenna the bat is- old func"""
    ...
    def find_bats_id(self): #need to be checked
        ### has a problem- when calling this class all dict becomes empty
        ## maybe change it all to df and save to csv read by main
        """ each read tag get specific key """
        if len(self.df) >= 1 & len(self.df) <=5 :
            ...
            print ("error")
        unique_tags  = last_bats['TAG'].unique()
        try:
            for tag in unique_tags:
                if tag not in self.tags:
                    bat_number = len(self.tags)+1
                    # self.bats_ids[bat_number] = tag
                    self.tags_ids[tag] = bat_number
                    self.tags.append(tag)
                self.bats_ids[self.tags_ids[tag]] = tag
                self.bats_loc = last_bats[tag]['ANTENNA'].tail(1)[0]
                    # self.tags[tag] = self.bat_id_{}.format(bat_number)
                    # self.f'bat_id_{bat_number}' = tag
        except:
            print ('tag error')

    # def find_bat_id (self): #need to be checked
    #     """ find id and location for more than one bat, acording to known tags (from define_all_tags)"""
    #     try:
    #         self.bats_ids = {}
    #         if len(self.df) >= 1 & len(self.df) <=5 :
    #             last_bats = self.df.tail(len(self.df))
    ...
    def run (self):
        ...
        if self.activity == True:
            # self.define_all_tags() #new
            self.find_bats_id() #new
