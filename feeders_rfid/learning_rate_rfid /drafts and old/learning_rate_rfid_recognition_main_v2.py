...
class Feeder:
    ...
    def __init__(self, fname):
        self.df = pd.DataFrame(columns=['signal','bat_id_1','bat_id_2','feeder','pump','condition'])
        self.disconnect = []
        # self.bat = False
        # self.cond = None
        self.fname = fname
        ...
        while time.time() < tr_end:
            ...
            while self.bat_loc == "no_bat":
                ...
                while time.time() < t_end:
                    ...
                    self.decide()
                    time.sleep(0.5)
                    if self.bat_loc != "no_bat" and flag_t == 0:
                        self.which_pump(R_p, L_p)
                        flag_t += 1
                if self.bat_loc != "no_bat":
                    break
