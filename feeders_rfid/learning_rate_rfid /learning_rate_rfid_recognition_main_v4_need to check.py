...
class Feeder:
    ...
    def signal_on_reward (self, rewarding_feeder = 'R', intervals = 20, running_time = 3200, R_p=(1,0), L_p=(1,0)):
        ...
        while time.time() < tr_end:
            ...
            while self.activity == False:
                ...
                # new_row = pd.Series(data = {'signal': 'play', 
                #                         'bat_id_1': self.bat_id_1, 'bat1_loc': self.loc_bat1,
                #                         'bat_id_2': self.bat_id_2, 'bat2_loc': self.loc_bat2, 'condition': self.cond}, 
                #                         index = pd.Timestamp.now().strftime('%d-%m-%Y-%H:%M:%S'))
                # new_row = pd.Series(data = {'time':pd.Timestamp.now().strftime('%d-%m-%Y-%H:%M:%S'), 'signal': 'play','condition': self.cond})
    ...
    def read_rfid (self):
            ...
            # print (self.loc_bat2)
            # new_row = pd.Series(data = {'time':pd.Timestamp.now().strftime('%d-%m-%Y-%H:%M:%S'), 
            #                             'bat_id_1': self.bat_id_1, 'bat1_loc': self.loc_bat1,
            #                             'bat_id_2': self.bat_id_2, 'bat2_loc': self.loc_bat2, 'condition': self.cond})
    ...
    def pump_it (self, pump_id, win_lose_p = (1,0)):
        ...
        try:
            ...
            if(val == "win"):
                ...
                # new_row = pd.Series(data = {'time':pd.Timestamp.now().strftime('%d-%m-%Y-%H:%M:%S'), 
                #                         'bat_id_1': self.bat_id_1, 'bat1_loc': self.loc_bat1,
                #                         'bat_id_2': self.bat_id_2, 'bat2_loc': self.loc_bat2, 
                #                         'pump': pump_id, 'condition': self.cond})
                # self.df = self.df.append(new_row, ignore_index = True)
                ...
                #                         'bat_id_1': self.bat_id_1, 'bat1_loc': self.loc_bat1,
                #                         'bat_id_2': self.bat_id_2, 'bat2_loc': self.loc_bat2, 
                #                         'pump': 'no {}'.format(pump_id), 'condition': self.cond})
                # self.df = self.df.append(new_row, ignore_index = True)
